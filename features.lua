...
local currentVelocity = vec3(0, 0, 0)
local lastVelocity = vec3(0, 0, 0)

-- 💥 DETECTOR DE IMPACTOS Y CONFIGURACIÓN DE DURABILIDAD MOTOR
    ...
    while true do
        ...
        if IsPedInAnyVehicle(ped, false) then
            ...
            if GetPedInVehicleSeat(veh, -1) == ped then
                ...
                -- Aplicación del Multiplicador de Daños del Config en tiempo real
                local currentEngineHealth = GetVehicleEngineHealth(veh)
                if currentEngineHealth < 1000.0 and lastEngineHealth then
                    local damageDone = lastEngineHealth - currentEngineHealth
                    if damageDone > 0 then
                        -- Modificamos el daño recibido según tu tabla protectora (ej: 0.3 = Triple duración)
                ...
                lastEngineHealth = GetVehicleEngineHealth(veh)

                -- Eyección física por choque sin cinturón
                if speedHUD >= (Config.MinSpeedEject or 60.0) then
                    local lastSpeed = #lastVelocity
                    local currentSpeed = #currentVelocity
                    local diff = lastSpeed - currentSpeed
        ...
        if GetPedInVehicleSeat(veh, -1) == ped then
            if GetVehicleEngineHealth(veh) > 150 then
                ...
                if engineStatus then
                    ...
                    lastEngineState = true -- EVITA EL BUCLE: Avisamos al client.lua que lo encendiste a propósito
                    lib.notify({title = 'Vehículo', description = 'Motor encendido.', type = 'success'})
                else
                    SetVehicleEngineOn(veh, false, false, true)
                    SetVehicleUndriveable(veh, true)
                    cruiseStatus = false
                    lastEngineState = false -- EVITA EL BUCLE: Avisamos al client.lua que lo apagaste a propósito
